...
export default function DashboardPage() {
  ...
  const locateMe = async (onResult: (address: string) => void) => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          onResult(address ?? `${latitude.toFixed(4)}, ${longitude.toFixed(4)}`);
        } catch {
      ...
      (err) => {
        console.error(err);
        onResult('Location access denied');
      },
                ...
                <div className="w-full bg-white p-6 rounded-3xl shadow-xl shadow-primary/5 border border-slate-100 mt-4">
                  ...
                  <div className="flex flex-col gap-4 mb-8">
                    <div className="relative group">
                      ...
                      />
                      <button
                        onClick={() => locateMe(setStartingPoint)}
